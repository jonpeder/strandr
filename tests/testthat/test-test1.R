...
test_that("test example strandr", {
  ...
  ex_out <- strandr(ex_in, lon = ex_in$Longitude, lat = ex_in$Latitude)
  expect_output(str(ex_out), "2 obs. of  10 variables")
})

test_that("test example stedsnavn", {
  ex_out <- stedsnavn(lon = c(13.191345, 23.186622), lat = c(65.834805, 70.44070))
  expect_output(str(ex_out), "2 obs. of  6 variables")
})

test_that("test example strandkoder", {
  ex_out <- strandkoder(lon = c(13.191345, 23.186622), lat = c(65.834805, 70.44070))
